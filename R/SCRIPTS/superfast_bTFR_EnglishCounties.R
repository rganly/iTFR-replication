...
#-----------------------------------
# Edited by Rachel Ganly
# Use Hong Kong Census Data 2016
...
#------------------------------------------------------
# Estimate 1:
# Estimate the TFR for 4 British counties in 1851 using bTFR model
# Vector C (total number of children under 5), Matrix W15...W45  
#------------------------------------------------------

county_names = 
  c("Leicestershire","Rutlandshire","Lincolnshire","Nottinghamshire","Derbyshire")
...
names(C_UK) = county_names

W_UK  = matrix(
  c(
  11729,	   11030,   9512,  8039,	 7009,  6228,	 5333,
   1119,	    1038,	   939,	  751,	  662,	 619,	  606,
  19259,	   17360,	 15479,	13211,	11511,	9952,	 8814,
  15069,	   14180,	 11920,	 9926,	 8881,	8165,	 6914,
  12993,	   12012,  10639,	 8827,	 7597,	7139,	 5950), 
     ncol=7, byrow=TRUE, 
     dimnames = list(county_names, seq(15,45,5) ) 
)

  
#approx: just assume all counties have similar mortality of 130/1000  
q5_UK = rep(.2, length(C_UK))  

# result will be a list with length= number of counties (67 for FL)
est        = superfast_bTFR(C=C_UK, W=W_UK, q5_est=q5_UK)
...
ggplot( data=df, aes(x=Q50, y=reorder(name, Q50))) +
    ...
    labs(x='TFR',y='County', 
         title='North Midlands Counties TFR',
...
#------------------------------------------------------
# Estimate 2:
# Estimate the TFR for 4 British counties in 1851 using bTFR model, with I-CEM data as inputs 
# Vector C (total number of children under 5), Matrix W15...W45  
#------------------------------------------------------
...
W_UK  = matrix(
  c(11959,11246,9609,8095,7028,6269,5375,
    1157,1061,960,767,673,624,606,
    19726,17707,15704,13346,11546,9982,8837,
    15065,14046,11692,9703,8602,7963,6709,
    13381,12266,10772,8918,7660,7187,5966), 
  ncol=7, byrow=TRUE, 
  dimnames = list(county_names, seq(15,45,5) ) 
)


#approx: just assume all counties have similar mortality 
q5_UK_low = rep(.1, length(C_UK))  
q5_UK_mid= rep(.15, length(C_UK))  
q5_UK_high= rep(.25, length(C_UK)) 

# result will be a list with length= number of counties (67 for FL)
est        = superfast_bTFR(C=C_UK, W=W_UK, q5_est=q5_UK_high)
names(est) = county_names
